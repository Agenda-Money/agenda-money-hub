...
export default function ApplyPage() {
  ...
  const handleRequestOtp = async () => {
    ...
    if (!nodeCode.trim()) { setErrorMessage("Enter your agent code."); return; }
    ...
    try {
      ...
      setNodeName(p?.nodeName ?? "your agent");
  ...
  if (view === "auth" || view === "otp") {
    return (
      <div className="min-h-screen bg-background">
        <PageHeader />
        <main className="max-w-sm mx-auto px-4 py-10 space-y-8">
          {/* Logo hero */}
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-3">
            <img src={agendaLogo} alt="Agenda Money" className="h-14 mx-auto rounded-xl" />
            <p className="text-sm text-muted-foreground">
              {view === "auth" ? "Enter your details to get started." : "Enter the code sent to your phone."}
            </p>
          </motion.div>
          ...
          <AnimatePresence mode="wait" custom={direction}>
            <motion.div key={view} custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit"
              ...
              {view === "auth" && (
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input value={msisdnInput} onChange={(e) => setMsisdnInput(e.target.value)}
                      type="tel" placeholder="024 XXX XXXX"
                      className="h-11 bg-muted/40 border-border/50 font-mono" />
                    <p className="text-xs text-muted-foreground">Must be your MoMo number</p>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-sm">Agent Code</Label>
                    <Input value={nodeCode} onChange={(e) => setNodeCode(e.target.value)}
                      placeholder="e.g. AM-1234" className="h-11 bg-muted/40 border-border/50 font-mono" />
                    <p className="text-xs text-muted-foreground">Provided by your agent</p>
                  </div>
                  <Button onClick={handleRequestOtp} disabled={!canSubmitEntry || isRequesting}
                    ...
                    {!isRequesting && <ArrowRight className="h-4 w-4 ml-2" />}
                  </Button>
                </div>
              )}
              ...
              {view === "otp" && (
                <div className="space-y-4">
                  <div className="bg-muted/40 rounded-lg p-3 text-sm text-center">
                    We'll verify with <strong className="text-foreground">{nodeName || "your agent"}</strong> for approval.
                  </div>
                  <div className="flex justify-center">
                    <InputOTP value={otp} onChange={(v) => { setOtp(v); if (v.length < OTP_LENGTH) resetAutoSubmit(); }} maxLength={OTP_LENGTH}>
                      <InputOTPGroup>
                        {OTP_SLOTS.map((slot, i) => <InputOTPSlot key={slot} index={i} />)}
                      </InputOTPGroup>
                    </InputOTP>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <Button onClick={handleVerifyOtp} disabled={!canVerify || isVerifying} className="flex-1 h-11 bg-primary text-primary-foreground hover:bg-primary/90">
                      {isVerifying ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : null}
                      {isVerifying ? "Verifying..." : "Verify"}
                    </Button>
                  </div>
                  <button onClick={handleResend} disabled={resendSeconds > 0 || isRequesting}
          ...
          <p className="text-center text-xs text-muted-foreground">🔒 Secure & encrypted</p>
        </main>
      </div>
    );
  }
