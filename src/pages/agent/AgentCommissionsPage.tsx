...
export default function AgentCommissionsPage() {
  ...
  const { data: historyData, isLoading: isHistoryLoading } = useQuery({
    queryKey: ["agent-commission-history"],
    queryFn: async () => {
      const res = await api.get("/api/agents/commissions");
      return res.data?.commissions?.items || [];
    }
  });

  const stats = summaryData || {
    signupCommission: 0,
    repaymentCommission: 0,
    defaultDeduction: 0,
    netEarnings: 0,
  };

  const totalEarned = (stats.signupCommission || 0) + (stats.repaymentCommission || 0);
  const pendingDeductions = Math.abs(stats.defaultDeduction || 0);
  const estimatedPayout = stats.netEarnings || 0;

  const commissions = historyData || [];
  
  const filteredHistory = commissions.filter((item: any) => {
    if (activeTab === "earnings") return item.amount > 0;
    if (activeTab === "deductions") return item.amount < 0;
    return true;
  });

  const getDescription = (type: string, reference?: string) => {
    if (type === "SIGNUP") return `New Customer Signup (${reference || "User"})`;
    if (type === "REPAYMENT") return `Loan Repayment Ref: ${reference || ""}`;
    if (type === "DEFAULT_DEDUCTION") return `Customer Default Ref: ${reference || ""}`;
    return type;
  };
        ...
        <Card className="bg-pink-50/50 dark:bg-pink-950/20 border-pink-200 dark:border-pink-900 shadow-sm">
          ...
          <CardContent>
            {isSummaryLoading ? (
               ...
               <>
                  <div className="text-3xl font-bold text-pink-900 dark:text-pink-100">GHS {estimatedPayout.toFixed(2)}</div>
        ...
        <Card>
          ...
          <CardContent className="pt-4">
            {isSummaryLoading ? (
               ...
               <>
                 <div className="text-3xl font-bold text-gray-900">GHS {totalEarned.toFixed(2)}</div>
                 <p className="text-xs text-green-600 font-medium mt-1 flex items-center gap-1">
                   {totalEarned >= 0 ? <><ArrowUpRight className="w-3 h-3" /> Looking good!</> : ""}
                 </p>
               </>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="pt-4">
             {isSummaryLoading ? (
               ...
               <>
                 <div className="text-3xl font-bold text-gray-900">GHS {pendingDeductions.toFixed(2)}</div>
               ...
               filteredHistory.map((item: any, index: number) => (
                 ...
                 >
                   <div className="flex items-center gap-4">
                     ...
                     <div>
                       <p className="text-sm font-bold text-gray-900">{getDescription(item.type, item.reference)}</p>
