...
export default function PendingKycPage() {
  ...
  const {
    ...
    isLoading: isFailedUsersLoading,
  } = useQuery({
  ...
  const { mutate: approveUser, isPending: isApproving } = useMutation({
    mutationFn: ({ msisdn, reason }: { msisdn: string; reason: string }) =>
      verifyUserKyc(msisdn, reason),
    onSuccess: async () => {
      ...
      try {
        const loan = await getUserActiveLoan(selectedUser.msisdn);
        if (loan?.data || loan?.id) {
          setPendingLoan(loan?.data || loan);
          setKycJustVerified(true);
        } else {
  ...
  const { mutate: rejectUser, isPending: isRejecting } = useMutation({
    mutationFn: ({ msisdn, reason }: { msisdn: string; reason: string }) =>
      failUserKyc(msisdn, reason),
    onSuccess: () => {
      toast.success("User KYC Rejected", {
        description:
          "The user has been moved to the failed/rejected queue and pending loans rejected.",
      });
      queryClient.invalidateQueries({ queryKey: ["pending-kyc-users"] });
      queryClient.invalidateQueries({ queryKey: ["pending-users-count"] });
    ...
    onError: (error: any) => {
      toast.error("Rejection Failed", {
        description:
          error.response?.data?.message || "Could not reject user KYC",
      });
    },
  });
  ...
  const { mutate: restoreUser, isPending: isRestoring } = useMutation({
    ...
    onError: (error: any) => {
      toast.error("Restoration Failed", {
        description:
          error.response?.data?.message || "Could not restore user KYC",
      });
    },
  });
  ...
  const handleApproveLoan = async () => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.message || "Failed to approve loan.");
    } finally {
  ...
  const searchFilteredUsers = (pendingUsers || []).filter(
    (user: any) =>
      user.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      user.msisdn?.includes(searchTerm),
  );

  const searchFilteredFailedUsers = (failedUsers || []).filter(
    (user: any) =>
      user.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      user.msisdn?.includes(searchTerm),
  );
                ...
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <CardTitle className="text-lg">
                      {deduplicateWords(user.fullName)}
                    </CardTitle>
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="pending" className="space-y-4">
          ...
          <TabsContent value="failed" className="mt-4">
            {renderUserList(
              failedList,
              "No failed KYC verifications found",
                ...
                <div className="space-y-6 py-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
                      ...
                      <div className="min-w-0">
                        <h2 className="text-xl sm:text-2xl font-black text-foreground truncate">
                          {deduplicateWords(selectedUser.fullName)}
                        </h2>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 pt-3 border-t border-border text-sm">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="font-mono font-medium text-xs break-all">
                          {selectedUser.ghanaCardNumber || "N/A"}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-muted/30 rounded-lg p-4 space-y-2 text-sm">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {selectedUser.accommodationType || "N/A"}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="font-medium">
                          {selectedUser.employmentStatus || "N/A"}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="font-medium">
                          {selectedUser.monthlyIncome || "N/A"}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {canWrite && (
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full">
                      {selectedUser.kycStatus === "FAILED" ||
                      selectedUser.kycStatus === "REJECTED" ? (
                        <Button
                          variant="outline"
                          onClick={() => {
                            const reason = prompt(
                              "Describe the reason for restoring this user to pending (required):",
                            );
                            if (reason?.trim())
                              restoreUser({
                      ...
                      {selectedUser.kycStatus !== "VERIFIED" && (
                        <Button
                          onClick={() => {
                            approveUser({
                              msisdn: selectedUser.msisdn,
                              reason: "Identity verified via admin dashboard",
                            });
                          }}
                  ...
                  {/* Pending Loan Alert after KYC Approval */}
                  {kycJustVerified && pendingLoan && (
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div>
                          ...
                          <p className="font-bold">
                            GHS {pendingLoan.amount?.toLocaleString() || "N/A"}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">
        ...
        <Dialog open={showRejectPrompt} onOpenChange={setShowRejectPrompt}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Please provide a reason for rejecting this KYC. Pending loans
